 ...
 * branch on `status`, so every surface in the product shows the same skeleton,
 * the same error card and the same retry affordance.
 */
...
export function useAsync<T>(
  ...
  const run = useCallback(async (isBackground: boolean) => {
    ...
    let tries = 0;
    // Backoff: 400ms, 800ms, 1600ms — matched to the transport's own timeouts.
    while (tries <= retries) {
      try {
